...
Broadly, analyses of the empirical datasets reproduce the patterns of bias across date resolution from from the simulation study in the reproductive number, substitution rate, and outbreak age (figures \ref{fig:emp-parm}). That is, the reproductive number increases with decreasing date resolution along with an increase in the substitution rate and corresponding decrease in the age of outbreak. There are a few exceptions to this trend that we consider below and which we attribute to the difference between simulated and empirical sampling time distributions.
...
Phylodynamic and phylogenetic likelihoods also appear diverged where the loss in date resolution exceed the average time for a substitution to arise (Figure \ref{fig:emp-likelihood}). For both viral datasets, month and day posterior distributions of phylodynamic and phylogenetic likelihood do not overlap, while all are overlapping for the \textit{M. tuberculosis} data. The \textit{S. aureus} data provide an intermediate case where only the posterior likelihoods for year-resolution differ. Together, these distributions support the hypothesis that date resolution larger than the average time to one mutation cause a shift in the likelihood manifold for analyses under both birth-death based and coalescent tree priors.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/empirical_likelihood.pdf}
    \caption{Posterior phylodynamic likelihood against phylogenetic likelihood for each combination of empirical dataset and with colour corresponding to date resolution. Ellipses represent the 95\% highest posterior density region. Both Phylodynamic and phylogenetic likelihoods are diverged between day and month resolution for the viral datasets, while year resolution differs from month and day for the \textit{S. aureus} data and posterior likelihoods all coincide for \textit{M. tuberculosis}. }
    \label{fig:emp-likelihood}
\end{figure}

\subsubsection*{H1N1}
Mean posterior $R_0$ increases with decreasing date resolution for both the birth death (1.08 to 1.14) yet remains similar for the coalescent exponential (1.14 to 1.13) (Table \ref{tab:emp-re}). Mean posterior substitution rate also decreases for both tree priors across day to month date resolution ($4.3\times10^{-3}$ to $3.9\times10^{-3}$ and $3.9\times10^{-3}$ to $3.2\times10^{-3}$ for the birth death and coalescent exponential respectively) (Table \ref{tab:emp-clock-age}). The posterior age of outbreak also differs between tree priors, with a decrease from date to month resolution for the birth death and an increase for the coalescent exponential. For the coalescent exponential, we can attribute the decrease in reproductive number and substitution rate from day to month resolution to month rounding spreading out the samples further in time, which drives the signal for and an older outbreak and lower rates of transmission overall. While the same is true for the birth-death, the additional information it draws from identical sampling times as month-resolution likely inflates the mean posterior reproductive number while still driving a lower substitution rate and outbreak age.
...
\begin{table}[]
    ...
    \begin{tabular}{lllcc}
        ...
        \midrule
        H1N1 & BD & Day & 4.3e-3 (3.7e-3, 4.9e-3) & 3.7e-1 (3.3e-1, 4.2e-1)\\
        H1N1 & BD & Month & 3.9e-3 (3.2e-3, 4.6e-3) & 3.5e-1 (3.1e-1, 4.1e-1)\\
        \addlinespace
        H1N1 & CE & Day & 3.9e-3 (3.2e-3, 4.5e-3) & 4.2e-1 (3.6e-1, 5.0e-1)\\
        H1N1 & CE & Month & 3.2e-3 (2.6e-3, 3.8e-3) & 4.6e-1 (3.8e-1, 5.6e-1)\\
        \addlinespace
        SARS-CoV-2 & BD & Day & 2.5e-4 (1.1e-4, 4.5e-4) & 1.4e-1 (1.4e-1, 1.5e-1)\\
        SARS-CoV-2 & BD & Month & 6.6e-4 (3.3e-4, 1.1e-3) & 1.7e-1 (1.7e-1, 1.7e-1)\\
        \addlinespace
        SARS-CoV-2 & CE & Day & 2.4e-4 (9.1e-5, 4.7e-4) & 2e-1 (1.4e-1, 3.6e-1)\\
        SARS-CoV-2 & CE & Month & 4.3e-5 (4.4e-6, 1.4e-4) & 1.6 (2.9e-1, 5.9)\\
        \addlinespace
        ...
        \addlinespace
        \textit{M. tuberculosis} & BD & Day & 1e-7 (6.5e-8, 1.4e-7) & 2.2e+01 (1.7e+01, 3.2e+01)\\
        \textit{M. tuberculosis} & BD & Month & 1e-7 (6.6e-8, 1.4e-7) & 2.2e+01 (1.7e+01, 3.2e+01)\\
        \textit{M. tuberculosis} & BD & Year & 9.9e-8 (6.2e-8, 1.4e-7) & 2.2e+01 (1.8e+01, 3.4e+01)\\
        \bottomrule
    \end{tabular}
...
\begin{table}[]
    ...
    \midrule
    H1N1 & BD & Day & 1.1 (1.1, 1.1) & - & -\\
    H1N1 & BD & Month & 1.1 (1.1, 1.2) & - & -\\
    \addlinespace
    H1N1 & CE & Day & 1.1 (1.1, 1.2) & - & -\\
    H1N1 & CE & Month & 1.1 (1.1, 1.2) & - & -\\
    \addlinespace
    SARS-CoV-2 & BD & Day & 1.2 (9.3e-1, 1.6) & - & -\\
    SARS-CoV-2 & BD & Month & 5.8 (3.7, 9.0) & - & -\\
    \addlinespace
    SARS-CoV-2 & CE & Day & 1 (9.6e-1, 1.0) & - & -\\
    SARS-CoV-2 & CE & Month & 1 (9.8e-1, 1.1) & - & -\\
    \addlinespace
    \textit{S. aureus} & BD & Day & - & 1.6 (1.5, 1.7) & 6.6e-1 (5.1e-1, 8.0e-1)\\
    \textit{S. aureus} & BD & Month & - & 1.6 (1.5, 1.7) & 6.8e-1 (5.4e-1, 8.3e-1)\\
    \textit{S. aureus} & BD & Year & - & 1.7 (1.6, 1.8) & 3.7e-1 (1.9e-1, 5.4e-1)\\
    \addlinespace
    \textit{M. tuberculosis} & BD & Day & - & 2.8 (5.8e-1, 5.3) & 1.4 (7.2e-1, 2.7)\\
    \textit{M. tuberculosis} & BD & Month & - & 2.7 (5.7e-1, 5.0) & 1.4 (7.4e-1, 2.7)\\
    \textit{M. tuberculosis} & BD & Year & - & 2.7 (4.6e-1, 5.1) & 1.5 (8.1e-1, 2.9)\\
    \bottomrule
    \end{tabular}
